...
def match_on_item_update(sender, instance, created, **kwargs):
    ...
    if instance.item_type == 'found':
        # Someone posted a found item → notify each lost item's owner
        for lost_item in matches:
            send_match_email(lost_item.reported_by, [instance])
        print(f"Notified {matches.count()} lost item owners about found Item {instance.id}")

    else:
        # Someone posted a lost item → notify them about existing found matches
        send_match_email(instance.reported_by, matches)
        print(f"Notified lost item reporter about {matches.count()} potential found matches for Item {instance.id}")